            ...
            if os.path.exists(sol_file):
                ...
                if os.path.exists(sol_file_default):
                    with open(sol_file_default, 'r', encoding='utf-8') as f:
                        sol_code = f.read()
                    print(f"成功读取反编译的Solidity代码(默认目录): {sol_file_default}")
                else:
                    print(f"警告: 未找到反编译的Solidity文件: {sol_file} 或 {sol_file_default}")
            ...
            if os.path.exists(abi_file):
                ...
                if os.path.exists(abi_file_default):
                    with open(abi_file_default, 'r', encoding='utf-8') as f:
                        abi_code = f.read()
                    print(f"成功读取ABI文件(默认目录): {abi_file_default}")
                else:
                    print(f"警告: 未找到ABI文件: {abi_file} 或 {abi_file_default}")
